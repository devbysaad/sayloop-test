...
const ResultScreen = ({ userId }: { userId: number }) => {
  ...
  // FIX: was reading 'result' — slice stores it as 'debateResult'
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-6 py-16 font-sans bg-[#fffbf5]">
      <div className="w-full max-w-lg animate-pop font-sans">
        ...
        {/* Result card */}
        <div className="rounded-[32px] p-8 mb-6 text-center border-2 shadow-2xl transition-all font-sans"
          ...
          <div className="grid grid-cols-3 gap-3">
            {[
              ...
              <div key={s.l} className="bg-white/60 backdrop-blur-xs rounded-2xl p-3.5 border transition-all hover:bg-white/80"
                style={{ borderColor: config.border }}>
        ...
        {/* Arguments recap */}
        {myArgs.length > 0 && (
          <div className="animate-fade-in-up [animation-delay:100ms] bg-white rounded-[28px] p-6 mb-6 border-2 border-gray-100/80 shadow-sm font-sans">
            ...
            <div className="space-y-3.5 max-h-52 overflow-y-auto pr-2">
              {myArgs.map((a: any, i: number) => (
                <div key={a.id} className="flex gap-4 items-start group">
                  <span className="w-5 h-5 rounded-lg flex items-center justify-center text-[10px] text-white shrink-0 mt-0.5 font-[900] bg-linear-to-br from-[#fbbf24] to-[#f97316] shadow-sm group-hover:scale-110 transition-transform">
                    {i + 1}
                  </span>
        ...
        {/* Action buttons */}
        <div className="animate-fade-in-up [animation-delay:200ms] flex gap-3.5 font-sans">
          {/* Go Home — just reset state, navigate handled by DebatePage status check */}
          <button
            onClick={() => dispatch(sessionActions.reset())}
            className="flex-1 py-4 rounded-2xl border-none bg-white text-gray-500 text-[13px] font-[900] uppercase tracking-wider hover:bg-gray-50 hover:-translate-y-0.5 transition-all shadow-sm active:scale-95 cursor-pointer"
          >
            Go back Home
          </button>

          {/* FIX: was calling sessionActions.connect({ userId }) — doesn't exist.
              Play Again just resets to idle; the user picks a topic again in IdleScreen. */}
          <button
            onClick={() => dispatch(sessionActions.reset())}
            className="flex-1 py-4 rounded-2xl text-white text-[13px] font-[900] uppercase tracking-wider hover:-translate-y-0.5 transition-all bg-linear-to-br from-[#fbbf24] to-[#f97316] shadow-[0_12px_28px_rgba(251,191,36,0.4)] hover:shadow-[0_18px_36px_rgba(251,191,36,0.5)] active:scale-95 border-none cursor-pointer"
          >
            Play Again 🎉
          </button>
        </div>
