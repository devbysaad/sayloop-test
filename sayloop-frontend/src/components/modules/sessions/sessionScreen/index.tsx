...
const timerColor = (secs: number | null) => {
  ...
  if (secs > 60)   return { text: '#16A34A', bg: '#F0FDF4', border: '#BBF7D0' }; // green
  if (secs > 30)   return { text: '#D97706', bg: '#FFFBEB', border: '#FDE68A' }; // yellow
  return             { text: '#DC2626', bg: '#FEF2F2', border: '#FECACA' };       // red
};
...
const XpPopups = ({ popups }: { popups: any[] }) => (
  <div className="absolute top-16 right-4 z-50 flex flex-col gap-2 items-end pointer-events-none">
    {popups.map(p => (
      <div key={p.id}
        className="flex items-center gap-2 rounded-2xl px-4 py-2.5 shadow-lg border-2 animate-bounce"
...
const SessionScreen = ({ userId }: Props) => {
  ...
  // ── Report mic changes to saga (on change only, not every tick) ──────────
  const prevMicRef = useRef<boolean>(!rtc.muted);
  useEffect(() => {
    const isOn = !rtc.muted;
    if (isOn !== prevMicRef.current) {
      console.log('[Session] Mic change detected → emitting mic:status isOn=', isOn);
      dispatch(sessionActions.reportMicStatus({ isOn }));
      prevMicRef.current = isOn;
    }
  }, [rtc.muted]);

  // ── Report speaking ticks (every 1s when mic is ON) ──────────────────────
  useEffect(() => {
    if (rtc.muted) return;
    const id = setInterval(() => {
      dispatch(sessionActions.reportSpeaking());
    }, 1000);
    ...
    <>
      ...
      <div className="sess h-screen flex flex-col overflow-hidden bg-slate-100 relative">
        ...
        {showResign && (
          <ResignModal
            onConfirm={() => {
              console.log('[Session] Resign confirmed → dispatching resign action');
              setShowResign(false);
        ...
        {/* ═══ 50/50 VIDEO AREA ═══ */}
        <div className="flex flex-col sm:flex-row flex-1 overflow-hidden gap-1 p-1 bg-slate-200" style={{ maxHeight:'calc(100vh - 220px)' }}>

          {/* LEFT — Opponent */}
          <div className="relative flex-1 rounded-2xl overflow-hidden bg-slate-800 shadow-lg min-h-[30vh] sm:min-h-0">
          ...
          {/* RIGHT — You */}
          <div className="relative flex-1 rounded-2xl overflow-hidden bg-slate-900 shadow-lg min-h-[30vh] sm:min-h-0">
        ...
        <div className="shrink-0 bg-white border-t-2 border-slate-100 shadow-[0_-4px_24px_rgba(0,0,0,0.06)]">
          ...
          {/* Chat panel */}
          {chatOpen && (
            <div className="slide-up border-t-2 border-slate-100 flex flex-col" style={{ height:'240px' }}>
