import {
  ...
  import {
    getBrowseUsers, requestMatch, acceptMatch, rejectMatch,
    getActiveMatches, getMatchHistory, getMatchById,
    type Match, type MatchUser,
  } from '../../lib/matchApi';
  import {
  ...
  function* handleLoadUsers(action: ReturnType<typeof matchActions.loadUsers>): Generator {
    ...
    try {
      const users = (yield call(getBrowseUsers, action.payload.userId)) as MatchUser[];
  ...
  function* handleSendRequest(action: ReturnType<typeof matchActions.sendRequest>): Generator {
    ...
    try {
      const result = (yield call(requestMatch, userId, partnerId, topic)) as any;
  ...
  function* pollMatchStatus(
    ...
    try {
      while (true) {
        yield delay(3000);
        const match = (yield call(getMatchById, matchId)) as Match;
  ...
  function* handleLoadRequests(action: ReturnType<typeof matchActions.loadRequests>): Generator {
    ...
    try {
      const all = (yield call(getActiveMatches, action.payload.userId)) as Match[];
  ...
  function* handleLoadHistory(action: ReturnType<typeof matchActions.loadHistory>): Generator {
    ...
    try {
      const res = (yield call(getMatchHistory, action.payload.userId)) as any;
  ...
  function* handleAcceptRequest(action: ReturnType<typeof matchActions.acceptRequest>): Generator {
    ...
    try {
      const result = (yield call(acceptMatch, matchId)) as any;
  ...
  function* handleRejectRequest(action: ReturnType<typeof matchActions.rejectRequest>): Generator {
    try {
      yield call(rejectMatch, action.payload.matchId);
  ...
  function* pollNotifications(action: ReturnType<typeof matchActions.startPollingNotifications>): Generator {
    try {
      while (true) {
        yield delay(5000);
        const active = (yield call(getActiveMatches, action.payload.userId)) as Match[];
